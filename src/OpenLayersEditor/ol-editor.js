...
(function () {
    ...
    var GeoMapEditorApp = (function () {
        ...
        function _updateField(fieldId, value) {
            ...
            const allFields = document.querySelectorAll("#" + fieldId);
            ...
            if (field) {
                ...
                console.log(
                    `✅ Campo ${fieldId} atualizado (${allFields.length} campos encontrados, usando último)`,
                );
            }
        }
        ...
        function _updateEditorConfigField() {
            ...
            const layerNames = Object.keys(_layerConfigs);
        ...
        /**
         * Restaura a configuração do editor
         * IMPORTANTE: Usa querySelectorAll para pegar o ÚLTIMO elemento com o ID
         * (mesma abordagem do OpenLayersMap que funciona)
         */
            ...
            try {
                ...
                if (config.layers) {
                    Object.keys(config.layers).forEach(function (name) {
                        ...
                        const index = Object.keys(_layerConfigs).indexOf(name);
                        ...
                        if (index !== -1 && _olLayers[index]) {
                            ...
                            if (settings.visible !== undefined) {
                                ...
                                // Atualiza checkbox - USA querySelectorAll com último
                                ...
                                // Atualiza slider de opacidade - USA querySelectorAll com último
                                const allSliders = document.querySelectorAll(
                                    "#layer_opacity_" + name,
                                );
                                if (allSliders.length > 0) {
                                    const slider =
                                        allSliders[allSliders.length - 1];
                                    if (slider) {
                                        slider.disabled = !settings.visible;
                                        slider.style.opacity = settings.visible
                                            ? "1"
                                            : "0.5";
                                    }
                                }
                ...
                // Atualiza estado do controle de camadas - USA querySelectorAll com último
                if (config.ui) {
                ...
                if (config.tools && config.tools.activeTool) {
                    ...
                    setTimeout(function () {
                        const allButtons = document.querySelectorAll(
                            ".ol-editor-toolbar-btn",
                        );
                        // Procura pelo botão que corresponde à ferramenta
                        allButtons.forEach(function (btn) {
                            const label = btn.querySelector(
                                ".ol-editor-toolbar-btn-label",
                            );
                            if (
                                label &&
