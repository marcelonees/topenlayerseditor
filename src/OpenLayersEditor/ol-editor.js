  ...
  var GeoMapEditorApp = (function () {
    ...
    function _createEditor() {
      ...
      if (ol.control && typeof ol.control.defaults === "function") {
        ...
        controls = [new ol.control.ScaleLine(), new ol.control.FullScreen()];
      }
      ...
      if (_config.showLayerControl) {
        const layerControl = _createLayerControl(_olLayers, layerConfigs);
    ...
    function _fitToGeometry(features, padding) {
      ...
      if (!_map || !features || features.length === 0) {
        console.warn("⚠️ Não é possível ajustar a visão: sem mapa ou features");
        return;
      }
    ...
    function _setupInteractions() {
      ...
      _draw = _createDrawInteraction("Polygon", _config.freehand || false);
      ...
      features.forEach(function (feature) {
        ...
        if (geom) {
          try {
            ...
            if (convertedGeom) {
              const geomJson = new ol.format.GeoJSON().writeGeometry(
                convertedGeom,
              );
              data.push({
                geometry: geomJson,
    ...
    function _restoreFromHistory(index) {
      ...
      state.features.forEach(function (item) {
        try {
          const geometry = new ol.format.GeoJSON().readGeometry(item.geometry);
    ...
    function _createLayerControl(olLayers, layerConfigs) {
      ...
      document.addEventListener("mousemove", function (e) {
        ...
        const mapContainer = document.getElementById(_config.containerId);
    ...
    function _addToolbar(container) {
      ...
      const positionStyle =
        positionStyles[_config.toolbarPosition] || positionStyles["top-right"];
      ...
      buttonConfigs.forEach(function (btn) {
        ...
        b.type = "button"; /* Garantir que o botão não submeta o formulário */
        ...
        if (btn.hasSubmenu) {
          ...
          drawTypes.forEach(function (item) {
            ...
            opt.onclick = function (e) {
              ...
              e.stopPropagation();
              console.log(
                "🎯 Tipo: " + item.type + " | Freehand: " + item.freehand,
              );

              submenu
                .querySelectorAll(".ol-editor-toolbar-submenu-item")
                .forEach(function (el) {
              ...
              _draw = _createDrawInteraction(item.type, item.freehand);
              if (_draw && typeof _draw.setActive === "function") {
              ...
              toolbar.querySelectorAll("button").forEach(function (btn) {
                if (
                  !btn.id ||
                  (btn.id !== "editor_undo_btn" && btn.id !== "editor_redo_btn")
                ) {
          ...
          Object.keys(config.layers).forEach(function (name) {
            ...
            if (index !== -1 && _olLayers[index]) {
              ...
              if (settings.visible !== undefined) {
                ...
                const checkbox = document.getElementById("layer_chk_" + name);
                if (checkbox) {
              ...
              if (settings.opacity !== undefined) {
                ...
                const slider = document.getElementById("layer_opacity_" + name);
                if (slider) {
